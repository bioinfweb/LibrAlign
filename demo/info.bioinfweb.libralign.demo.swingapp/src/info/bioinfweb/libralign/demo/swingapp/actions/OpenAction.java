		...
		if (handleUnsavedChanges()) {
			try {
				if (getOpenFileChooser().showOpenDialog(getEditor().getFrame()) == JFileChooser.APPROVE_OPTION) {
					...
					AlignmentDataReader mainReader = new AlignmentDataReader(eventReader, new BioPolymerCharAlignmentModelFactory('?', false));  // All unknown tokens (which are not nucleotide, amino acid, ambiguity or gap) will be set to '?'.
					mainReader.readAll();
