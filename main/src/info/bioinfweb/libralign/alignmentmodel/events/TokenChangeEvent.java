 ...
 */
public class TokenChangeEvent<T> extends AbstractPartModelEvent<T> {
	 ...
	 */
	protected TokenChangeEvent(AlignmentPartModel<T> partModel, int sequenceID, ListChangeType type,
			int startIndex, Collection<? extends T> affectedTokens) {
		
		super(partModel, sequenceID, type, startIndex);
	 ...
	 */
	public static <T> TokenChangeEvent<T> newInsertInstance(AlignmentPartModel<T> partModel, int sequenceID,
			int startIndex, Collection<? extends T> newTokens) {
		
		return new TokenChangeEvent<T>(partModel, sequenceID, ListChangeType.INSERTION, startIndex, newTokens);
	}
	...
	public static <T> TokenChangeEvent<T> newInsertInstance(AlignmentPartModel<T> partModel, int sequenceID,
		...
		return newInsertInstance(partModel, sequenceID, index, Collections.nCopies(1, newToken));
	}
	 ...
	 * @return a new instance with the same token type as {@code source}
	 */
	public static <T> TokenChangeEvent<T> newRemoveInstance(AlignmentPartModel<T> partModel, int sequenceID,
			int startIndex, Collection<? extends T> removedTokens) {
		
		return new TokenChangeEvent<T>(partModel, sequenceID, ListChangeType.DELETION, startIndex, removedTokens);
	}
	...
	public static <T> TokenChangeEvent<T> newRemoveInstance(AlignmentPartModel<T> partModel, int sequenceID,
		...
		return newRemoveInstance(partModel, sequenceID, index, Collections.nCopies(1, removedToken));
	}
	 ...
	 * @return a new instance with the same token type as {@code source}
	 */
	public static <T> TokenChangeEvent<T> newReplaceInstance(AlignmentPartModel<T> partModel, int sequenceID,
			int startIndex, Collection<? extends T> replacedTokens) {
		
		return new TokenChangeEvent<T>(partModel, sequenceID, ListChangeType.REPLACEMENT, startIndex, replacedTokens);
	}
	...
	public static <T> TokenChangeEvent<T> newReplaceInstance(AlignmentPartModel<T> partModel, int sequenceID,
		...
		return newReplaceInstance(partModel, sequenceID, index, Collections.nCopies(1, replacedToken));
	}


	/**
	 * Returns the (former) index of the first affected token relative to the start of the part model where it happened.
	 * 
	 * @return a value greater or equal to zero (The first token of sequence has the index 0.)
	 * @see #getGlobalStartIndex()
	 */
	public int getPartStartIndex() {
		return getIndex();
	}
	
	
	/**
	 * Returns the (former) index of the first affected token relative to the start of the whole (concatenated) alignment.
	 * 
	 * @return a value greater or equal to zero (The first token of sequence has the index 0.)
